...
func SetupConfigPath() {
	...
	_, testEnvFound := os.LookupEnv("usersvc_TESTENV")
	_, prodEnvFound := os.LookupEnv("usersvc_PRODENV")

	configFileName, configFileFound := os.LookupEnv("usersvc_CONF")
	...
	if !testEnvFound && !prodEnvFound {
		...
		panic("FATAL :: Config file env path usersvc_CONF not set in production or test environment. Can't start service")
	}
